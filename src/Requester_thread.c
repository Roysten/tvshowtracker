...
static void handle_request_search_show(ULONG_PTR data)
{
    ...
    swprintf(request_uri_buf, request_uri_buf_len, L"/3/search/tv?api_key=%ls&language=en-US&page=1&query=%ls", REST_API_KEY, urlencoded_buf);
	free(urlencoded_buf);
...
static void handle_request_show_info(ULONG_PTR data)
{
    ...
    swprintf(request_uri_buf, ARRAYSIZE(request_uri_buf), L"/3/tv/%" PRIi64 "?api_key=%ls&language=en-US", context_info->show_id, REST_API_KEY);
    LOG("Request: %ls", request_uri_buf);
	...
	for (int i = 0; i < show->seasons.len; ++i) {
		...
		swprintf(request_uri_buf, ARRAYSIZE(request_uri_buf), L"/3/tv/%" PRIi64 "/season/%d?api_key=%ls&language=en-US", show->id, season->season_number, REST_API_KEY);
		LOG("Request: %ls", request_uri_buf);
